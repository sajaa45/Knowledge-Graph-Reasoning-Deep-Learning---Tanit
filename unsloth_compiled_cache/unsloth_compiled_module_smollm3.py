...
from typing import Any, List, Optional, Tuple, Union, Dict, Set, Callable
from transformers.models.smollm3.modeling_smollm3 import (__name__, Callable, Optional, Union, torch, nn, ACT2FN, Cache, GenerationMixin, use_kernel_forward_from_hub, FlashAttentionKwargs, BaseModelOutputWithPast, CausalLMOutputWithPast, ROPE_INIT_FUNCTIONS, dynamic_rope_update, ALL_ATTENTION_FUNCTIONS, PreTrainedModel, Unpack, TransformersKwargs, can_return_tuple, deprecate_kwarg, SmolLM3Config, SmolLM3PreTrainedModel, SmolLM3Model, SmolLM3ForCausalLM)
...
class SmolLM3Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        return SmolLM3Attention_forward(self, hidden_states, position_embeddings, attention_mask, past_key_values, cache_position, **kwargs)


@torch.compile(fullgraph = True, dynamic = True, options = torch_compile_options)
def SmolLM3RMSNorm_forward(self, hidden_states):
...
class SmolLM3RMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        return SmolLM3RMSNorm_forward(self, hidden_states)
...
class SmolLM3MLP(nn.Module):
    ...
    def forward(self, x):
        return SmolLM3MLP_forward(self, x)
...
class SmolLM3RotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        return SmolLM3RotaryEmbedding_forward(self, x, position_ids)
...
class SmolLM3ForCausalLM(SmolLM3PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        return SmolLM3ForCausalLM_forward(self, input_ids, attention_mask, position_ids, past_key_values, inputs_embeds, labels, use_cache, cache_position, logits_to_keep, **kwargs)
