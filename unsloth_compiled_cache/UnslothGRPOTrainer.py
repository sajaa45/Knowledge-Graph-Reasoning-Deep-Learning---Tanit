...
from trl.trainer.grpo_trainer import (Any, AutoConfig, AutoModelForSequenceClassification, AutoProcessor, AutoTokenizer, BaseTrainer, DataLoader, Dataset, FSDP, GRPOConfig, GRPOTrainer, GenerationConfig, IterableDataset, Path, PeftConfig, PreTrainedModel, PreTrainedTokenizerBase, ProcessorMixin, RepeatSampler, RewardFunc, RolloutFunc, Sampler, SyncRefModelCallback, TrainerCallback, VLLMClient, _ForwardRedirection, apply_chat_template, broadcast_object_list, datasets, defaultdict, deque, disable_dropout_in_model, ensure_master_addr_port, entropy_from_logits, gather, gather_object, get_config_model_id, identity, inspect, is_conversational, is_datasets_available, is_liger_kernel_available, is_peft_model, is_rich_available, is_vllm_available, logger, logging, nanmax, nanmin, nanstd, nn, nullcontext, os, pad, partial, pd, prepare_deepspeed, prepare_fsdp, prepare_multimodal_messages, prepare_multimodal_messages_vllm, prepare_peft_model, print_prompt_completions_sample, profiling_context, profiling_decorator, seed_worker, selective_log_softmax, set_seed, shuffle_sequence_dict, split_pixel_values_by_grid, split_tensor_dict, textwrap, torch, transformers, unsplit_pixel_values_by_grid, unwrap_model_for_generation, wandb, warnings, Any, apply_chat_template, gather, gather_object, is_conversational, logging, nanmax, nanmin, nanstd, os, pad, prepare_multimodal_messages, torch, FSDP, apply_chat_template, broadcast_object_list, gather, gather_object, is_conversational, nullcontext, os, pad, pd, prepare_multimodal_messages, prepare_multimodal_messages_vllm, profiling_context, torch, transformers, unwrap_model_for_generation, entropy_from_logits, os, pad, selective_log_softmax, torch, transformers, Any, profiling_decorator, shuffle_sequence_dict, split_pixel_values_by_grid, split_tensor_dict, torch, unsplit_pixel_values_by_grid, PreTrainedModel, logger, os, torch, FSDP, nn, os, pd, FSDP, nn, pd, torch, GRPOTrainer, gather, nanmax, nanmin, os, torch)
    ...
    def _generate_single_turn(self, prompts: list):
        ...
        if self.use_vllm:
            ...
            # Generate completions using colocated vLLM instances: each device holds vLLM copy and work on their own batch of prompts
                ...
                with profiling_context(self, "vLLM.generate"):
                    if is_conversational({"prompt": prompts[0]}):
                        all_outputs = self.llm.chat(all_prompts, sampling_params=sampling_params, use_tqdm=False, lora_request = self.model.load_lora('grpo_trainer_lora_model', load_tensors = True))
                    else:
                        all_outputs = self.llm.generate(all_prompts, sampling_params=sampling_params, use_tqdm=False, lora_request = self.model.load_lora('grpo_trainer_lora_model', load_tensors = True))
